...
struct ContentView: View {
        ...
        ZStack {
            ...
            HStack(spacing: 10) {
                ...
                Text("v\(appVersion)")
                    ...
                    .foregroundColor(.secondary)
                Spacer()
                Link(destination: URL(string: "https://github.com/jiayouzl/StandUp/")!) {
    ...
    private var timerModeContent: some View {
        ...
        Divider()
        SettingsRow(icon: "figure.cooldown", iconColor: .indigo, title: "休息活动") {
        ...
        VStack(alignment: .leading, spacing: 10) {
            Toggle(isOn: $timer.isPreReminderEnabled) {
                Label("提前提醒 - 进入遮挡页前10秒铃声提醒", systemImage: "speaker.wave.2.fill")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
            }
            .toggleStyle(.checkbox)

            Toggle(isOn: $timer.isAutoPauseEnabled) {
                Label("自动暂停 - 进入遮挡页后暂停当前媒体播放", systemImage: "pause.circle.fill")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
            }
            .toggleStyle(.checkbox)
